...
import 'home_search_suggestion_tile.dart';

/// Модальное окно поиска по каталогу.
class HomeSearchModal extends ConsumerStatefulWidget {
  const HomeSearchModal({super.key});

  static Future<void> show(BuildContext context) => showDialog<void>(
    context: context,
    barrierDismissible: true,
    barrierColor: Colors.black.withValues(alpha: 0.12),
    builder: (_) => const HomeSearchModal(),
  );

  @override
  ConsumerState<HomeSearchModal> createState() => _HomeSearchModalState();
}

class _HomeSearchModalState extends ConsumerState<HomeSearchModal> {
  ...
  void _onQueryChanged() {
    ref
        .read(HomeDi.homeSearchProvider.notifier)
  ...
  Widget build(BuildContext context) {
    ...
    final searchState = ref.watch(HomeDi.homeSearchProvider);
...
class _ResultsBody extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (!searchState.hasQuery || searchState.status == HomeSearchStatus.idle) {
    ...
    if (searchState.status == HomeSearchStatus.empty ||
        searchState.status == HomeSearchStatus.error) {
      return _StatusMessage(
        screenSize: screenSize,
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return HomeSearchSuggestionTile(
